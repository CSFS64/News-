  ...
  _cardHTML: function (a) {
    ...
    return '<div class="news-card m-card" data-id="' + a.id + '">' +
      '<div class="card-body">' +
        ...
        (a.images&&a.images.length ? '<div class="art-img-grid art-img-grid--'+Math.min(a.images.length,3)+'" data-imgs="'+esc(JSON.stringify(a.images))+'">'+a.images.map(function(u,i){return u.match(/\.(mp4|webm|mov)(\?|$)/i) ? '<video src="'+esc(u)+'" class="art-img-thumb" muted playsinline preload="metadata" data-idx="'+i+'" style="object-fit:cover"></video>' : '<img src="'+esc(u)+'" alt="" loading="lazy" data-idx="'+i+'" class="art-img-thumb">';}).join('')+'</div>' : '') +
